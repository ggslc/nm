...
def make_adv(h, dt, gamma=1e4, A=1):
    ...
    def adv(u, d, d_old):
        ...
        source = gamma * A * dx * ((0.5 * mu_nl * dudx - hd)**4) #remove rho*g to be consistent with force balance #makes no difference because I've set these to 1...
        #there might be something about source term linearisation I've forgotten about... Patankar said somethibg...
        source = source.at[-1].set(0)
        #print(source)
...
def make_adv_linear(h, dt):

    def adv(u, d, source_coefficient, d_old):
        hd = h*d
        hd_old = h*d_old

        source = source_coefficient * d + epsilon
...
def make_picard_iterator_full(mu_centres_zero, h, beta, dt, iterations):
    ...
    def iterate(state):
        ...
        #update d
        adv_vto = jac_adv_fn(u, d, d_old) #Makes more sense to have this here than before updating u
...
#Doesn't work. Stupid idea.
def make_picard_iterator_full_newton_for_d(mu_centres_zero, h, beta, dt, iterations, newton_its=10):
    ...
    def inner_newton(i, for_state):
        u, d, d_old = for_state

        jac_adv = jac_adv_fn(u, d, d_old)
        d = d.at[:-1].set(d[:-1] + lalg.solve(jac_adv[:-1, :-1], -adv(u, d, d_old)[:-1]))
        d = jnp.minimum(d, 0.95)
        d = jnp.maximum(d, 0)
    ...
    def iterate(state):
        ...
        #update d with newton iteration
        u, d, d_old = jax.lax.fori_loop(0, newton_its, inner_newton, (u, d, d_old))
...
def make_picard_iterator_most_basic(mu_centres_zero, h, beta, dt, iterations):
    ...
    ###for debugging:
    ##jac_vto = jac_vto_fn(u_trial, d_trial, new_mu(u_trial))
    #mu, dudx = new_mu(u_trial)
    #source = new_source_term(d_trial, mu, dudx)
    #jac_adv = jac_adv_fn(u_trial, d_trial, d_trial.copy())
    #print(source)
    #print(-adv(u_trial, d_trial, d_trial)[:-1])
    #print(lalg.solve(jac_adv[:-1, :-1], source[:-1]))
    ##print(jac_vto)
    #print(jac_adv)
    #raise
    ...
    def iterator(u_init, d_init):
        ...
        for i in range(iterations):
            ...
            #update u
            u = u + lalg.solve(jac_vto_fn(u, d, mu), -vto(u, d, mu))
...
def make_picard_iterator_most_basic_compiled(mu_centres_zero, h, beta, dt, iterations):
    ...
    def iterate(state):
        ...
        return u, mu, d, d.copy(), residual, i+1
    ...
    def iterator(u_init, d_init):
        ...
        initial_state = u, mu, d, d_old, 1, 0

        u, mu, d, d_old, residual, itns = jax.lax.while_loop(condition, iterate, initial_state)
...
def make_picard_iterator_fully_linear(mu_centres_zero, h, beta, dt, iterations, newton_its=10):
    ...
    def new_source_coefficient(d, mu_nl, dudx,gamma=1e4, A=1):
        #not sure whether it's best to interpolate these onto cell centres or whether it's best
        #to just define new ones on the cell centres...
        ...
        source_coeff = gamma * A * dx * (((stress_cc - h*d)**4)/(d + epsilon))
    ...
    ##jac_vto = jac_vto_fn(u_trial, d_trial, new_mu(u_trial))
    #mu, dudx = new_mu(u_trial)
    #source_coeff = new_source_coefficient(d_trial, mu, dudx)
    #jac_adv = jac_adv_fn(u_trial, d_trial, source_coeff, d_trial.copy())
    #print(source_coeff)
    #print(-adv(u_trial, d_trial, source_coeff, d_trial)[:-1])
    #print(lalg.solve(jac_adv[:-1, :-1], -adv(u_trial, d_trial, source_coeff, d_trial)[:-1]))
    ##print(jac_vto)
    #print(jac_adv)
    #raise
    ...
    def iterate(state):
        ...
        #update linear damage source coefficient
        source_coeff = new_source_coefficient(d, mu, dudx)
        
        #update d
        jac_adv = jac_adv_fn(u, d, source_coeff, d_old)
        d = d.at[:-1].set(d[:-1] + lalg.solve(jac_adv[:-1, :-1], -adv(u, d, source_coeff, d_old)[:-1]))
        d = jnp.minimum(d, 0.95)
        d = jnp.maximum(d, 0)
        ...
        residual1 = jnp.max(jnp.abs(vto(u, d, mu)))
        residual2 = jnp.max(jnp.abs(adv(u, d, d, d_old)))
        residual_combo = residual1+residual2
...
while number <= 1:
    ...
    number *= 2
timestep_lengths = numbers
timesteps = [int(total_time/tl) for tl in timestep_lengths]


for tl, ts in zip(timestep_lengths, timesteps):
    iterator = make_picard_iterator_most_basic_compiled(jnp.ones_like(u_trial), h, beta, tl, 20)
    ...
    plotboths(ds, us, 30, title=None, savepath="./plots/no_adv_length_{}_timesteps_{}.png".format(tl, ts), axis_limits=None, show_plots=False)
