...
def solve_advection_by_newtons_method(h_init, u, dt, dx, n, num_timesteps, num_iterations):
    ...
    for j in range(num_timesteps):
        ...
        if not j%5:
            plot_h(h, jnp.zeros_like(h)*jnp.nan, "implicit_fou_{}".format(j))
...
def plot_h(h, h_analytic, id_):
    ...
    plt.plot(h, color='b')
    plt.ylim(-0.5,1.5)
    plt.savefig("../misc/advec_tests/implicit/ppm_test_fou_{}.png".format(id_))
    plt.close()
...
if __name__ == "__main__":
    ...
    plot_h(h, h_analytic, "implicit_fou_100")
